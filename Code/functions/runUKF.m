        ...
        switch info.val_type 
            case "openloop"
                switch info.modelstr
                    ...
                    case "GPR_UKF"
                        x_est(:, k) = par.fx(x_est(k-1), EFC(k));
                end
            case "UKF"
                switch info.modelstr
                    case "linear"
                    ...
                    case "GPR_UKF"
                        [x_pred_k, x_mean_k, P_k] = state_prediction(info, x_k, P_k, L, lambda, par, EFC(k), [], Wm, Wc);  
                end
...
function [x_pred, x_mean, P] = state_prediction(info, x_prev, P, L, lambda, par, EFC, dEFC, Wm, Wc)
% State prediction
    ...
    switch info.modelstr
        case "linear"
            x_pred = par.fx(sigma_points, EFC, dEFC);
        case "GPR_UKF"
            x_pred = par.fx(sigma_points', EFC*ones(length(sigma_points), 1));
            x_pred = x_pred';
    end
...
function [x, P] = state_update(x_mean, z_meas, par, P, K, S)
    % update state estimate and covariance
    innovation = z_meas - par.fz(x_mean);
